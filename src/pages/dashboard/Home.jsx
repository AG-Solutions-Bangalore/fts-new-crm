  ...
  const fetchResult = async () => {
    try {
      ...
      if (response.status == "200") {
        ...
        const barLabels = response.data?.graph1.map(item => item.receipt_donation_type) || [];
            ...
            <div className="bg-white rounded-lg shadow-sm overflow-hidden">
              <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                ...
                <div className="flex gap-2">
                  <button onClick={() => togglePanel('notices')} className="p-2 hover:bg-gray-100 rounded-lg">
                    <Minimize2 className="h-5 w-5 text-gray-500" />
                  </button>
                  <button onClick={() => setVisiblePanels(prev => ({...prev, notices: false}))} className="p-2 hover:bg-gray-100 rounded-lg">
                  ...
                  datanotification.map((notice) => (
                    <div key={notice.id} className="p-6">
                      <div className="flex justify-between items-start mb-2">
                        <h3 className="font-semibold text-gray-900">{notice.notice_name}</h3>
                        {notice.is_read == 0 ? (
                          <span className="px-2 py-1 bg-blue-50 text-blue-600 text-xs rounded-full">New</span>
                        ) : (
                          <span className="px-2 py-1 bg-green-50 text-green-600 text-xs rounded-full">Read</span>
                        )}
                      </div>
                      <p className="text-gray-600 text-sm mb-3">{notice.notice_detail}</p>
                      <div className="flex justify-between items-center">
                        ...
                        {notice.is_read == 0 && (
                          <button
                            onClick={() => {
                              if (window.confirm("Confirm that you have read this notice?")) {
          ...
          {visiblePanels.receiptsPie && (
            <div className="bg-white rounded-lg shadow-sm overflow-hidden">
              <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                ...
                <div className="flex gap-2">
                  <button onClick={() => togglePanel('receiptsPie')} className="p-2 hover:bg-gray-100 rounded-lg">
                    <Minimize2 className="h-5 w-5 text-gray-500" />
                  </button>
                  <button onClick={() => setVisiblePanels(prev => ({...prev, receiptsPie: false}))} className="p-2 hover:bg-gray-100 rounded-lg">
                    <X className="h-5 w-5 text-gray-500" />
                  </button>
                </div>
              </div>
          ...
          {visiblePanels.totalDonation && (
            <div className="bg-white rounded-lg shadow-sm overflow-hidden">
              <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                ...
                <div className="flex gap-2">
                  <button onClick={() => togglePanel('totalDonation')} className="p-2 hover:bg-gray-100 rounded-lg">
                    <Minimize2 className="h-5 w-5 text-gray-500" />
                  </button>
                  <button onClick={() => setVisiblePanels(prev => ({...prev, totalDonation: false}))} className="p-2 hover:bg-gray-100 rounded-lg">
                    <X className="h-5 w-5 text-gray-500" />
                  </button>
                </div>
              </div>
